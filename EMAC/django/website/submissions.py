...
class SubmissionForm(ModelForm):
    ...
    class Meta:
        model = Submission
        fields = [
            'submitter_first_name', 'submitter_last_name', 'submitter_email', 'name', 'credits', 'description',
            'subtitle', 'version', 'search_keywords', 'code_languages', 'logo_image', 'logo_link', 
    ...
    def clean_name(self):
        ...
        if self.instance is None or self.instance.id is None: # If this is a new Submission, not an edit ...
            if Resource.objects.filter(name=resource_name).exists() or Submission.objects.filter(name=resource_name).exists():
                raise ValidationError(_("An EMAC resource name must be unique, please enter a different name"))
...
def email_strings_for(submission, save_type, changed_fields, curator):
    ...
    if save_type is SaveType.SUBMIT:
        ...
        admin_message = str(f"A web user has submitted a new resource to EMAC via https://{domain}/submissions/\n\n")
        admin_message += submission_text
        admin_message += str(f"\n\nTo review and manage this submission, go to https://{domain}/admin/website/submission/")
    
        submitter_message = "Thank you for submitting a new resource to the Exoplanet Modeling & Analysis Center! All new resources are reviewed for readiness and alignment with the EMAC strategic goals before they are posted and/or prepared for further development. The EMAC team will review your submission and contact you for further information if needed.\n\n"
        submitter_message += submission_text
        submitter_message += str(f"\n\nIf you would like to review or revise your submission you may do so here: https://{domain}/submissions/{submission.id}/")
    ...
    # previous version.
        ...
        if "submitter_first_name" in changed_fields:
            if changed_fields["submitter_first_name"][0] != "":
                admin_submission_text += "OLD submitter_first_name: " + changed_fields["submitter_first_name"][0] + "\n"
            admin_submission_text += "NEW submitter_first_name: " + changed_fields["submitter_first_name"][1] + "\n\n"
        else:
            admin_submission_text += "submitter_first_name: " + submission.submitter_first_name + "\n\n"

        if "submitter_last_name" in changed_fields:
            if changed_fields["submitter_last_name"][0] != "":
                admin_submission_text += "OLD submitter_last_name: " + changed_fields["submitter_last_name"][0] + "\n"
            admin_submission_text += "NEW submitter_last_name: " + changed_fields["submitter_last_name"][1] + "\n\n"
        else:
            admin_submission_text += "submitter_last_name: " + submission.submitter_last_name + "\n\n"

        if "submitter_email" in changed_fields:
            if changed_fields["submitter_email"][0] != "":
                admin_submission_text += "OLD submitter_email: " + changed_fields["submitter_email"][0] + "\n"
            admin_submission_text += "NEW submitter_email: " + changed_fields["submitter_email"][1] + "\n\n"
        else:
        ...
        for field in changed_fields:
            if field == "name" or field == "submitter_first_name" or field == "submitter_last_name" or field == "submitter_email":
                pass
            else:
        ...
        if save_type is SaveType.EDIT:
            admin_message = str(f"A web user has revised a submission via https://{domain}/submissions/{submission.id}/\n\n")
        ...
        submitter_message = "Thank you for revising your submission to the Exoplanet Modeling & Analysis Center! All revised submissions are reviewed for readiness and alignment with the EMAC strategic goals before they are posted and/or prepared for further development. The EMAC team will review your submission and contact you for further information if needed.\n\n"
        submitter_message += submission_text
        submitter_message += str(f"\n\nIf you would like to further review or revise your submission you may do so here: https://{domain}/submissions/{submission.id}")
    
    elif save_type is SaveType.FIRSTCONTACT:
        subject = "Listing Software on the NASA Exoplanet Modeling & Analysis Center"
        
        context = {'submitter_first_name': submission.submitter_first_name,
...
def submission_was_saved(submission, save_type, changed_fields=None, curator=None):
    ...
    subject, admin_message, submitter_message = email_strings_for(submission, save_type, changed_fields, curator)
    ...
    if settings.EMAC_DOMAIN.startswith("emac.gsfc"):
        requests.post(settings.EMAC_SUBMISSION_SLACK_URL, data=data.encode('utf-8'), headers=headers)
        ...
        if save_type is SaveType.FIRSTCONTACT or save_type is SaveType.RECONTACT or save_type is SaveType.FINALCONTACT or save_type is SaveType.INITIALINLITCONTACT or save_type is SaveType.SECONDINLITCONTACT or save_type is SaveType.FINALINLITCONTACT:
            send_mail(subject, submitter_message, from_address, [str(submission.submitter_email)], fail_silently=False, html_message=submitter_message)
        else:
        ...
        requests.post(settings.EMAC_SUBMISSION_TEST_SLACK_URL, data=data.encode('utf-8'), headers=headers)
...
def edit(request, id):
    ...
    if request.POST:
        ...
        if submission_form.is_valid():
            ...
            rectify_categories_for(submission)
            if submission.status in [SubmissionStatus.CONTACTED.name, SubmissionStatus.IN_LITERATURE.name, SubmissionStatus.REJECTED_ABANDONED.name]:
                submission.status = SubmissionStatus.RECEIVED.name
            submission.save()
...
def resource_update_alert(request, id):
    ...
    if request.POST:
        submission_form = ResourceUpdateForm(request.POST, request.FILES, instance=Submission.objects.get(id=id))
