...
Django management command to import a SKOS TTL into ControlledGraphList rows.

- Creates a ControlledGraphList node for each concept occurrence in the tree.
- If a concept has multiple parents, a *separate* ControlledGraphList row (new 
  UUID) is created for each parent edge and attached as that parent's child.
- Guards against cycles.

Assumptions:
- <your_app>.models defines ControlledGraphList and its ManyToMany "children"
...
def build_maps(graph: Graph):
	# Collect all skos:Concepts
	concepts: Set[URIRef] = set(graph.subjects(RDF.type, SKOS.Concept))
	# Include any nodes that participate in broader/narrower even if not typed
	concepts.update(set(graph.subjects(SKOS.broader, None)))
	concepts.update(set(graph.objects(None, SKOS.broader)))
	concepts.update(set(graph.subjects(SKOS.narrower, None)))
	concepts.update(set(graph.objects(None, SKOS.narrower)))

	# Labels/defs
	labels: Dict[URIRef, str] = {c: best_label(graph, c) for c in concepts}
	defs: Dict[URIRef, Optional[str]] = {c: best_definition(graph, c) for c in concepts}
...
def parse_ttl(
	model: type[ControlledGraphList], 
	file_url: str, 
	remove_only_matched: bool = False,
	kill_single_root: bool = False
):
	"""
	...
	try: graph = load_graph(file_url)
	except Exception as e:
		raise CommandError(f"Failed to parse TTL: {e}")

	concepts, labels, defs, parents, children = build_maps(graph)
	...
	elif kill_single_root and len(roots) == 1:
		sole = roots[0]
		roots = sorted(children.get(sole, set()), key=lambda c: labels.get(c, str(c)).lower())
		children.pop(sole)

	created = 0
	for root in sorted(roots, key=lambda c: labels.get(c, str(c)).lower()):
		row = build_tree_and_persist(root, labels, defs, parents, children, None, None, model)
		if row is not None:
			created += 1
	...
	# remove old objects
	if remove_only_matched:
		for obj in matched_old_objs:	
			obj.delete()
	else:
		for obj in old_objs:	
			obj.delete()
